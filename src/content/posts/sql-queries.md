...
excerpt: Встраиваем SQL запросы в Data Science проект.
thumbnail: images/posts/sql-queries/thumbnail.jpg
minutes: 11
---
...
Доступ к полной бухгалтерии магазинов закрыт. Мы можем обращаться только к одной базе данных: в нее ежедневно загружается часть данных о продажах. Отработаем все примеры на SQLite. Если хотите повторять все действия вместе со мной, скачайте и распакуйте файл [↓ data.db.zip](/blog/static/posts/sql-queries/data.db.zip). 
...
```python title="train.py" {2-3,13-17}
import pandas as pd
