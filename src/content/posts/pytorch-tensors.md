...
excerpt: Разбираем тензорные вычисления с помощью PyTorch.
thumbnail: images/posts/pytorch-tensors/thumbnail.jpg
minutes: 28
---
...
![Виды тензоров](/blog/images/posts/pytorch-tensors/tensor_types.png)
...
Для начала установим PyTorch. Для этого разработчики рекомендуют выбрать нужную команду через конфигуратор на [официальном сайте](https://pytorch.org/get-started/locally/). Но если у вас нет видеокарты от NVidia или вы не хотите сейчас устанавливать на нее драйвера с поддержкой CUDA, будет достаточно выполнить команду:
...
![Случайное разбиение](/blog/images/posts/pytorch-tensors/train_test_split.png)
...
![Случайное разбиение с перемешиванием](/blog/images/posts/pytorch-tensors/train_test_split_shuffle.png)
...
![Нормализация](/blog/images/posts/pytorch-tensors/standard_scaler.png)
...
![Хранение тензоров и списков в памяти](/blog/images/posts/pytorch-tensors/array_storage.png)
...
В любом из случаев вы можете рассмотреть альтернативный вариант — [Google Colab](https://colab.google) или [Kaggle](https://kaggle.com). Обе площадки дают бесплатный доступ к видеокартам через Jupyter ноутбуки с подготовленной средой и установленными библиотеками. Там этот код выполнится без проблем.
...
![Градиентный спуск: общая идея](/blog/images/posts/pytorch-tensors/gradient_descent_idea.png)
...
![Градиентный спуск](/blog/images/posts/pytorch-tensors/gradient_descent.png)
...
$$
\text{Оценка} = \mathbf{w}_1 \cdot \text{Алкоголь} + \mathbf{w}_2 \cdot \text{Кислотность} + \dots + b.
$$
...
Получаем примерно полбалла. Это неплохой результат, но вы можете попробовать улучшить его. Для этого можно изменить количество шагов градиентного спуска, коэффициент обучения или использовать модель с большим количеством слоев. Пример обучения двухслойной модели можете найти в файле [↓ model.py](/blog/static/posts/pytorch-tensors/model.py).
