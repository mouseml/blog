...
thumbnail: images/posts/fastapi-services/thumbnail.jpg
minutes: 15
---
...
Для примера возьмем набор данных [соревнованияТ-Банка](https://www.kaggle.com/competitions/fintech-credit-scoring/data) по кредитному скорингу. Посмотрите на его описание и скажите, что вы видите?
...
![Архитектура проекта](/blog/images/posts/fastapi-services/architecture.png)
...
Данные в таком виде уже подготовлены в файле [↓ scoring.csv](/blog/static/posts/fastapi-services/scoring.csv), а полный код проекта [доступен на GitHub](https://github.com/mouseml/credit_scoring).
...
![Пример микросервисной архитектуры](/blog/images/posts/fastapi-services/services.png)
...
Если думаете, что это сложно, то нет. Вы пользуетесь HTTP запросами каждый день. Когда вы заходите на YouTube, вы отправляете HTTP запрос по адресу [youtube.com](https://youtube.com), а в ответ получаете HTML документ, который отображается в вашем браузере. 
...
![Страница Swagger UI](/blog/images/posts/fastapi-services/swagger.png)
...
```python title="service.py" {1,13,17-18}
import joblib
from fastapi import FastAPI
from pydantic import BaseModel
...
Для этого используем фреймворк [Streamlit](https://www.streamlit.io). Тут сразу оговорюсь — это не настоящая front-end разработка, а скорее быстрый способ создать красивый прототип. То есть как раз наш случай.
...
![Страница Swagger UI](/blog/images/posts/fastapi-services/interface-1.png)
...
![Страница Swagger UI](/blog/images/posts/fastapi-services/interface-2.png)
...
![Страница Swagger UI](/blog/images/posts/fastapi-services/interface-3.png)

Несмотря на все упрощения, можно сказать, что именно таким образом ML модели интегрируются в абсолютное большинство реальных продуктов. Текущую версию проекта можете закоммитить на GitHub или наоборот, забрать [мою версию](https://github.com/mouseml/credit_scoring).
...
В этом случае вы будете совмещать [body параметры](https://fastapi.tiangolo.com/tutorial/body/) и [query параметры](https://fastapi.tiangolo.com/tutorial/query-params/). Поэтому чтобы лучше понимать, что происходит, советую про них прочитать в [официальной документации](https://fastapi.tiangolo.com) FastAPI — она доступна на всех языках. Также изучите виды запросов (`GET`, `POST`, `PUT` и `DELETE`) и статус коды. Это необходимый минимум, который пригодится вам в дальнейшем.
...
Поэтому добавляйте сайт в закладки, подписывайтесь на [YouTube канал](https://www.youtube.com/channel/UCscWjyvPudzdIaGCCtEL3nw) и заходите пообщаться в [Telegram](https://www.t.me/ml_mouse). Увидимся в код ревью, всем пока!
